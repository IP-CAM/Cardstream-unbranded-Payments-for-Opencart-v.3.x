...
class ControllerExtensionPaymentPaymentNetwork extends Controller
{
    ...
    private function createDirectForm($includeDeviceDetails = false) {
        ...
        $data['button_confirm'] = $this->language->get('button_confirm');
        $data['process_url'] = $this->url->link(self::$url . '/direct_form_process', '', true);
    ...
    public function direct_form_process() {
        $redirectUrl = $this->url->link(self::$url . '/direct_callback', '', true).'&XDEBUG_SESSION_START=session_name';
        ...
        if (isset($_POST['threeDSVersion'])) {
            $parameters['threeDSVersion'] = $_POST['threeDSVersion'];
        }
